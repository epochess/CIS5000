...
Definition polychrome (c : color) : bool := 
  match (monochrome c) with
  | true => false
  | false => true
  end.

Example egpolychrome1:    (polychrome white) = false.
...
Fixpoint minusnat (x y : nat) : nat := 
  match x, y with
  | _, O => x
  | O, _ => O 
  | S x', S y' => minusnat x' y'
  end.
...
Proof.
  ...
  - intro H. reflexivity.
  - intro H. reflexivity.
  - rewrite lower_letter_F_is_F. simpl. intro H. rewrite H. reflexivity.
Qed.   
...
Proof.
  ...
  - destruct l.
    ...
    + reflexivity.
    + reflexivity.
    + rewrite lower_grade_F_Minus. simpl. intro H. rewrite H. reflexivity.
Qed.   
...
Proof.
  intros. rewrite apply_late_policy_unfold. rewrite H. rewrite H0. reflexivity.
Qed. 
...
Fixpoint bin_to_nat_helper (n: nat) (m:bin) : nat :=
  match m with
  | Z => O
  | B1 x => exp 2 n + bin_to_nat_helper (n + 1) x
  | B0 x => bin_to_nat_helper (n + 1) x
  end.

Definition bin_to_nat (m : bin) : nat := bin_to_nat_helper (0 : nat) (m : bin).
